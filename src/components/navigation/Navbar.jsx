...
const Navbar = () => {
    ...
    return (
        <div className='z-9 flex fixed justify-between w-full'>
            <div className='w-[120px] p-4'>
                <svg className='w-full h-full' fill='white' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103 44">
            ...
            <div onMouseEnter={()=> navGreenRef.current.style.height = "100%"} onMouseLeave={()=> navGreenRef.current.style.height = "0%"} className='relative bg-black w-[215px] h-[50px]'>
                <div ref={navGreenRef} className='bg-[#D3FD50] transition-all cursor-pointer h-0 w-full absolute top-0'></div>
                <div className='relative h-full lg:px-4 px-4 flex flex-col justify-center items-end lg:gap-1.5 gap-0.5'>
                    <div className="lg:w-12 w-12 h-0.5 bg-white"></div>
                    <div className="lg:w-6 w-6 h-0.5 bg-white"></div>
                </div>
            </div>
        </div>
    )
